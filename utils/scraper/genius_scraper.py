    ...
    for i, row in mini_df.iterrows():
        try:
            song = LyricsGenius.search_song(
                title=row['title'], artist=row['band_singer'])  # search for lyrics
            ...
            print("Failed to search for song: {0} by {1}".format(
                row['title'], row['band_singer']))
            df.loc[row['index'], 'song_id'] = None
...
billboard_all_lyrics = pd.read_csv('data/billboard_song_ids.csv').merge(
    all_lyrics, how='left', on='song_id').dropna().drop(columns=['index'])
