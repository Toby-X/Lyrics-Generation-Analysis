...
def retrieve_lyrics(song_id):
    ...
    URL = "http://genius.com/" + 'songs/' + str(song_id)
    print(URL)
    ...
    try:
        lyrics = html.find("div", class_="lyrics").get_text()  # todo: this may cause error when NoneType has not attribute get_text()
    except AttributeError as e:
        print(f'cannot fine {path}')
        print(e)
        return None
    return lyrics
...
def main():
    ...
    # Gets information regarding the artist themselves
    # followers = search_artist(artist_id)

    # Shows some random songs from artist and lyrics
    search(term)
